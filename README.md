...
The `Images` workflow rebuilds and tests `v3.0` and `v3.1` every Sunday at 05:17 Europe/Zurich, then publishes both architectures to the three registries. Its schedule lives on `main` and dispatches the full pipeline on each release branch. APT updates are picked up during rebuilding; explicitly pinned tools are updated manually. See [build instructions](https://github.com/whatwedo/docker-base-images/blob/v3.1/build.md).
